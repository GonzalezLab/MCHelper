...
Manual Curation Helper Version 1.6.0
Novelties:
  * Changed the BEE method by extension by saturation method.
...
def new_module1(plots_dir, ref_tes, gff_files, outputdir, pre, te_aid, automatic, minDomLTR, num_copies, minFLNA,
                verbose, repet, blastn_db, blastx_db, tools_path, ref_profiles, library_path):
    kept_seqs_record = []
    seqs_to_module3 = []
    non_curated = []
    seqs_manu = 0
    ...
    if maf is None:
        ...
        if maf.shape[0] > min_cluster:  # Don't try to cluster if less than min_cluster sequences
            ...
            if maf.shape[0] > max_sequences:  # If more than max_sequences get a sample of them
                ...
                d = K2Pdistance(maf)
                # print("[+++] Sampling", max_sequences, "sequences from the alignment.")
...
def process_maf(r, kd, min_plurality, end_threshold, fasta_table_ite, te_class):
    if r is None:
        return None
    else:
        stats = []
        ...
        stats.append(pd.DataFrame({
            ...
            'maf_kd': [kd],
            'cluster_kd': [kdc],
            'end_l': [r[r['ID'] < minmin_r.loc[0, 'ID']].shape[0] - np.sum((r[r['ID'] < minmin_r.loc[0, 'ID']]['res']+r[r['ID'] < minmin_r.loc[0, 'ID']]['mb'])==seqs) - (np.sum(r[r['ID'] < minmin_r.loc[0, 'ID']]['res'])/seqs) > end_threshold],
            'end_r': [r[r['ID'] > minmin_r.loc[min_l-1, 'ID']].shape[0] - np.sum((r[r['ID'] > minmin_r.loc[min_l-1, 'ID']]['res']+r[r['ID'] > minmin_r.loc[min_l-1, 'ID']]['mb'])==seqs) - (np.sum(r[r['ID'] > minmin_r.loc[min_l-1, 'ID']]['res'])/seqs) > end_threshold]
        }))

        # add or modify the consensus to the fasta_table
        if name in [te_ids for te_ids in fasta_table_ite.loc[:, "seq"]]:  # Consensus already in the fasta_table
            index = [te_ids for te_ids in fasta_table_ite.loc[:, "seq"]].index(name)
            fasta_table_ite.loc[index, "cons"] = s
        else:  # it's a new subfamily
            fasta_table_ite = pd.concat([fasta_table_ite, pd.DataFrame({"seq": name, "cons": s, "cons_size": [len(s)], "class": te_class, "end_l": stats[0].loc[0, 'end_l'], "end_r":  stats[0].loc[0, 'end_r']})], ignore_index=True)

    return stats, fasta_table_ite
...
def extension_by_saturation(genome, tes_to_extend, exe_nucl, num_ite, outputdir, max_nns, id_thread, min_perc_model, min_cluster, max_sequences, cluster_factor, group_outliers, min_plurality, end_threshold):
    ...
    if len(tes_to_extend) > 0:
        ...
        fasta_table = pd.DataFrame({"seq": str(tes_to_extend[0].id.split("#")[0]), "cons": str(tes_to_extend[0].seq).lower(), "cons_size": [len(tes_to_extend[0].seq)], "class": str(tes_to_extend[0].id).split("#")[1], "end_l": False, "end_r": False})
        for i in range(1, len(tes_to_extend)):
        ...
        while ite < num_ite and not all_finished:
            ...
            for index in range(fasta_table.shape[0]):
                if not fasta_table.loc[index, "end_l"] or not fasta_table.loc[index, "end_r"]:
                    ...
                    if copies_to_use > 2:  # if there is at least three hit of the consensus and the genome
                        ...
                        if len(mafft_comm_output) > 0:
                            ...
                            maffts_df_list, kd = seq_clus(mafft_df, min_cluster, max_sequences, cluster_factor, group_outliers)
                            for mafft in maffts_df_list:
                                # to do: modify to add the new subfamilies or modify exist ones
                                stats, fasta_table_ite = process_maf(mafft, kd, min_plurality, end_threshold, fasta_table_ite, te_class)
                                for k in range(len(stats)):
                                    stats_all_list_ite.append(stats[k])
            ...
            if len(stats_all_list_ite) > 0:
                stats_all = stats_all_list_ite[0].iloc[:, :]
                """for i in range(1, len(stats_all_list_ite)):
                    stats_all = pd.concat([stats_all, stats_all_list_ite[i].iloc[:, :]])
                with open(outputdir + '/stats_round_' + str(ite + 1) + '.tsv', 'a') as f:
        ...
        for index in range(fasta_table.shape[0]):
            ...
            # plot the MSA
            if os.path.exists(outputdir + "/" + str(fasta_table.loc[index, "seq"]) + ".copies.mafft"):
                output = subprocess.run(
                    ['CIAlign', '--infile', outputdir + "/" + str(fasta_table.loc[index, "seq"]) + ".copies.mafft", '--out',
                     outputdir + "/" + str(fasta_table.loc[index, "seq"]) + ".copies.cialign", '--plot_output', '--silent'])
...
def run_BEE_parallel(genome, ref_tes, exe_nucl, num_ite, outputdir, max_nns, cores, tool_path):
    create_output_folders(outputdir)
    ...
    localresults = [pool.apply_async(BEE,
                                     args=[genome, tes[ini_per_thread[x]:end_per_thread[x]], exe_nucl, num_ite,
                                           outputdir, max_nns, x, tool_path]) for x in range(cores)]
...
def BEE(genome, ref_tes, exe_nucl, num_ite, outputdir, max_nns, id_thread, tool_path):
    final_results = []
    num_seq = 0
    min_perc_model = 0.9  # only used copies longer than 80% consensus' length
    for te in ref_tes:
        seq_name = str(te.id).split("#")[0]
        if len(str(te.id).split("#")) > 1:
            te_order = str(te.id).split("#")[1].upper()
        else:
            te_order = "Unknown"

        if "LINE" in te_order or "SINE" in te_order:
            # Be more flexible with LINEs since they have truncated copies (mainly in 5`)
            min_perc_model = 0.9
        else:
            min_perc_model = 0.9
        ...
        for it in range(num_ite):
            ...
            # parameters from Anna Protasio github
            output = subprocess.run(
                ['blastn', '-query', outputdir + "/" + str(seq_name) + ".copies.cialign_consensus.fasta", '-db', genome,
                 '-out', outputdir + "/" + str(seq_name) + ".blast", '-num_threads', "1", "-outfmt", "6",
                 "-evalue", "1e-20"], stdout=subprocess.PIPE, text=True)
            # Second step EXTRACT and EXTEND
            blastresult = pd.read_table(outputdir + "/" + str(seq_name) + ".blast", sep='\t',
            ...
            result_file = open(outputdir + "/" + str(seq_name) + ".copies.fa", "w")
            original_cons = [consen for consen in SeqIO.parse(outputdir + "/" + str(seq_name) + ".copies.cialign_consensus.fasta", "fasta")][-1]
            ...
            while hit < blastresult.shape[0] and hit < 20:
                ...
                ini_hit = int(blastresult.at[hit, 'sstart'])
                end_hit = int(blastresult.at[hit, 'send'])
                if int(blastresult.at[hit, 'length']) > len(te.seq) * min_perc_model:
            ...
            if hit > 2:  # if there is at least three hit of the consensus and the genome
                # parameters from Anna Protasio's github
                output = subprocess.run(['mafft', '--quiet', '--thread', '1', '--reorder',
                                         outputdir + "/" + str(seq_name) + '.copies.fa'],
                                        stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True)
                ...
                if len(mafft_comm_output) > 0:
                    # test if using specific parameters by order improve the CIAlign's crop
                    if "LTR" in te_order or "TRIM" in te_order or "LARD" in te_order:
                        output = subprocess.run(
                            [tool_path + '/CIAlign-TE_new/build/lib/CIAlign/CIAlign', '--infile',
                             outputdir + "/" + str(seq_name) + ".copies.mafft", '--out',
                             outputdir + "/" + str(seq_name) + ".copies.cialign",
                             '--plot_output', '--remove_divergent', '--remove_divergent_minperc', '0.4',
                             '--remove_insertions', '--insertion_max_size', '5000', '--crop_ends',
                             '--crop_ends_redefine_perc', '0.5', '--crop_ends_mingap_perc', '0.05',
                             '--crop_ltrs', '--crop_ltrs_minscore', '150', '--crop_ltrs_window_size', '200',
                             '--remove_short', '--make_consensus', '--silent'],
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                    elif "TIR" in te_order or "MITE" in te_order:
                        output = subprocess.run(
                            [tool_path + '/CIAlign-TE_new/build/lib/CIAlign/CIAlign', '--infile',
                             outputdir + "/" + str(seq_name) + ".copies.mafft", '--out',
                             outputdir + "/" + str(seq_name) + ".copies.cialign",
                             '--plot_output', '--remove_divergent', '--remove_divergent_minperc', '0.5',
                             '--remove_insertions', '--insertion_max_size', '5000', '--crop_ends',
                             '--crop_ends_redefine_perc', '0.5', '--crop_ends_mingap_perc', '0.05',
                             '--crop_tirs', '--crop_tirs_mingap', '100', '--crop_tirs_minlen', '6',
                             '--crop_tirs_maxlen', '50', '--remove_short', '--make_consensus', '--silent'],
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                    elif "LINE" in te_order or "SINE" in te_order:
                        output = subprocess.run(
                            [tool_path + '/CIAlign-TE_new/build/lib/CIAlign/CIAlign', '--infile',
                             outputdir + "/" + str(seq_name) + ".copies.mafft", '--out',
                             outputdir + "/" + str(seq_name) + ".copies.cialign",
                             '--plot_output', '--remove_divergent', '--remove_divergent_minperc', '0.4',
                             '--remove_insertions', '--insertion_max_size', '5000', '--crop_ends',
                             '--crop_ends_redefine_perc', '0.5', '--crop_ends_mingap_perc', '0.05', '--crop_divergent',
                             '--crop_divergent_auto', '--crop_divergent_buffer_size', '50',
                             '--make_consensus', '--silent'],
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                    else:
                        output = subprocess.run(
                            [tool_path + '/CIAlign-TE_new/build/lib/CIAlign/CIAlign', '--infile',
                             ...
                             '--remove_insertions', '--insertion_max_size', '5000', '--crop_ends',
                             '--crop_ends_redefine_perc', '0.5', '--crop_ends_mingap_perc', '0.05', '--crop_divergent',
                             '--crop_divergent_auto', '--crop_divergent_buffer_size', '50', '--remove_short',
                             '--make_consensus', '--silent'],
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                    ...
                    if os.path.exists(outputdir + "/" + str(seq_name) + ".copies.cialign_removed.txt"):
                        delete_files(outputdir + "/" + str(seq_name) + ".copies.cialign_removed.txt")
                        delete_files(outputdir + "/" + str(seq_name) + ".copies.cialign_cleaned.fasta")
                        delete_files(outputdir + "/" + str(seq_name) + ".copies.cialign_aligned_LTRs.fasta")
                        delete_files(outputdir + "/" + str(seq_name) + ".copies.cialign_LTRplot.png")
                        delete_files(outputdir + "/" + str(seq_name) + ".copies.cialign_aligned_LTRs.fasta")
                        delete_files(outputdir + "/" + str(seq_name) + ".copies.cialign_aligned_TIRs.fasta")

                    new_consensus = [con for con in SeqIO.parse(outputdir + "/" + str(seq_name) +
                                    ".copies.cialign_consensus.fasta", 'fasta')][-1]
                    delete_files(outputdir + "/" + str(seq_name) + ".copies.cialign_consensus.fasta")
                    write_sequences_file([new_consensus],
                                         outputdir + "/" + str(seq_name) + ".copies.cialign_consensus.fasta")
                delete_files(outputdir + "/" + str(seq_name) + ".copies.mafft")

        final_seq = [con for con in SeqIO.parse(outputdir + "/" + str(seq_name) + ".copies.cialign_consensus.fasta", 'fasta')][-1]

        if len(final_seq.seq) > 0:
...
if __name__ == '__main__':
    ...
    if module in [1, 123]:
        ...
        if not os.path.exists(outputdir + "/classifiedModule/kept_seqs_classified_module_curated.fa"):
            ...
            ############################################################################################################
            # Second step: reduce redundancy with cd-hit
            ############################################################################################################
            start_time = time.time()
            ref_tes = run_cdhit(outputdir + "/classifiedModule/extended_cons.fa", outputdir + "/classifiedModule/", cores)
            end_time = time.time()
            if verbose:
